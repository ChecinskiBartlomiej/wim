...
def main():
    ...
    print(f"FORCED CPU MODE - Using device: {device}")
    print("(GPU will NOT be used even if available)")

    # Hyperparameters - LARGER network for better GPU comparison
    input_size = 10
    hidden_size = 4092  # Much larger! (was 32)
    ...
    print(f"\nInitializing LARGER MLP with architecture: {input_size} -> {hidden_size} -> {hidden_size} -> {hidden_size} -> {output_size}")
    print(f"(4 layers instead of 3, hidden size {hidden_size} instead of 32)")
    ...
    output_file = "test_results_cpu.txt"
    with open(output_file, 'w') as f:
        f.write(f"Device: {device}\n")
        f.write(f"Mode: FORCED CPU (GPU not used)\n")
