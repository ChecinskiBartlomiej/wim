...
def train(cfg, optimizer_name="Adam"):
    ...
    # Initialize EMA (Exponential Moving Average) - DDPM paper uses decay=0.9999
    ...
    for epoch in range(cfg.num_epochs):
        ...
        for imgs in tqdm(dataloader):
            ...
            # Update EMA after optimizer step (as in DDPM paper)
            ema.update(model)
        ...
        if (epoch + 1) in cfg.checkpoint_epochs:
            ...
            # Use EMA weights for generation (better quality as in DDPM paper)
            model.eval()
            ema.apply_shadow(model)  # Switch to EMA weights
            with torch.no_grad():
            ...
            # Calculate FID score (still using EMA weights)
