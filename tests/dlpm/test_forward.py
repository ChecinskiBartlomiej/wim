...
def test_forward_corruption():
    """
    Visualize forward diffusion process - image corruption over time using alpha-stable noise.
    """
    ...
    print("\n" + "="*60)
    print("FORWARD DIFFUSION VISUALIZATION (DLPM)")
    print("="*60)
    ...
    # Generate alpha-stable noise once (same noise for all timesteps for fair comparison)
    noise = dlpm.sample_alpha_stable(size=x_0.shape)
    ...
    for t_val in timesteps:
        if t_val == 0:
            ...
            x_t = dlpm.forward(x_0, noise, t)
        corrupted_images.append(x_t)

    # Create visualization
    fig, axes = plt.subplots(2, 5, figsize=(20, 8))
    fig.suptitle(f'Forward Diffusion Process (DLPM - Alpha-Stable Noise, alpha={cfg.alpha})', fontsize=16)

    axes = axes.flatten()

    for idx, (t_val, x_t) in enumerate(zip(timesteps, corrupted_images)):
        # Convert to image format using built-in function
        img = dlpm.tensor_to_image(x_t, cfg)[0]  # Returns list, get first image
