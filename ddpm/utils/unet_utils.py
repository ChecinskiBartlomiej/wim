...
class DownC(nn.Module):
    ...
    def __init__(self, in_channels: int,
        ...
        self.conv2 = nn.ModuleList([
            NormActConv(out_channels, out_channels) for _ in range(num_layers)
        ]) 
...
class UpC(nn.Module):
    ...
    def __init__(self,
        ...
        self.te_block = nn.ModuleList([
            TimeEmbedding(out_channels,
                          t_emb_dim) for _ in range(num_layers)
        ])
        ...
        self.up_block = Upsample(in_channels,
                                 in_channels//2) if up_sample else nn.Identity()
